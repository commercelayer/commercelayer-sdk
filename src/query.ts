...
type QueryArraySortable<R extends Resource> = Array<
  QueryResourceSortableFields<R> | `-${QueryResourceSortableFields<R>}`
>
...
const generateQueryStringParams = <R extends Resource>(
  params: QueryParams<R> | undefined,
    ...
    if (params.filters) {
      Object.entries(params.filters).forEach(([p, v]) => {
        ...
        if (Array.isArray(v)) {
          if (!isArrayFilter(p))
            throw new SdkError({
              message: `Incorrect filter [${p}]: Array value is supported only for the following filters: ${arrayFilters.join(', ')}`,
          ...
          if (!isObjectFilter(p))
            throw new SdkError({
              message: `Incorrect filter [${p}]: Object value is supported only for the following filters: ${objectFilters.join(', ')}`,
              type: ErrorType.REQUEST,
            })
...
const generateSearchString = (params?: QueryStringParams, questionMark: boolean = true): string => {
  ...
  return `${questionMark ? '?' : ''}${Object.entries(params)

...
class Voids extends ApiResource<Void> {
  ...
  async reference_authorization(
    ...
    return this.resources.fetch<Authorization>(
      { type: 'authorizations' },
      `voids/${_voidId}/reference_authorization`,
      params,
      options,
  ...
  async _forward(id: string | Void, params?: QueryParamsRetrieve<Void>, options?: ResourcesConfig): Promise<Void> {
    return this.resources.update<VoidUpdate, Void>(
      { id: typeof id === 'string' ? id : id.id, type: Voids.TYPE, _forward: true },
      params,
      options,
    )
  }

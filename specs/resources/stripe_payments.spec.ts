...
import { CommerceLayerClient, StripePayment, stripe_payments, orders } from '../../lib'
...
describe('StripePayments resource', () => {
  ...
  const resourcePath = 'stripe_payments'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await stripe_payments.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

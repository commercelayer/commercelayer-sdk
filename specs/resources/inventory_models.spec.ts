...
import { CommerceLayerClient, InventoryModel, inventory_models } from '../../lib'
...
describe('InventoryModels resource', () => {
  ...
  const resourcePath = 'inventory_models'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(inventory_models)
		expect(inventory_models.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await inventory_models.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
import { CommerceLayerClient, TalonOneAccount, talon_one_accounts } from '../../src'
...
describe('TalonOneAccounts resource', () => {

  const resourceType = 'talon_one_accounts'
  const resourcePath = 'talon_one_accounts'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(talon_one_accounts)
		expect(talon_one_accounts.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await talon_one_accounts.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
import { CommerceLayerClient, EventCallback, event_callbacks } from '../../lib'
...
describe('EventCallbacks resource', () => {
  ...
  const resourcePath = 'event_callbacks'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(event_callbacks)
		expect(event_callbacks.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.webhook', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'webhook')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '.event_stores', async () => {
		...
		await event_callbacks.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
import { CommerceLayerClient, RecurringOrderCopy, recurring_order_copies, orders, order_subscriptions } from '../../lib'
...
describe('RecurringOrderCopies resource', () => {

  const resourceType = 'recurring_order_copies'
  const resourcePath = 'recurring_order_copies'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await recurring_order_copies.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

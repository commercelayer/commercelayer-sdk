...
describe('CheckoutComGateways resource', () => {
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(checkout_com_gateways)
		expect(checkout_com_gateways.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await checkout_com_gateways.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '.checkout_com_payments', async () => {
	...
	test(resourceType + '._disable', async () => {
	  ...
	  const id = TestData.id
		...
		await checkout_com_gateways._disable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '._enable', async () => {
	  ...
	  const id = TestData.id
		...
		await checkout_com_gateways._enable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

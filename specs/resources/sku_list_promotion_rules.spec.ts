...
import { CommerceLayerClient, SkuListPromotionRule, sku_list_promotion_rules, percentage_discount_promotions, sku_lists } from '../../lib'
...
describe('SkuListPromotionRules resource', () => {

  const resourceType = 'sku_list_promotion_rules'
  const resourcePath = 'sku_list_promotion_rules'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(sku_list_promotion_rules)
		expect(sku_list_promotion_rules.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await sku_list_promotion_rules.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
import { CommerceLayerClient, Sku, skus, shipping_categories, tags } from '../../lib'
...
describe('Skus resource', () => {
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(skus)
		expect(skus.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await skus.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

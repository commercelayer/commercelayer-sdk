...
describe('AdyenGateways resource', () => {

  const resourceType = 'adyen_gateways'
  const resourcePath = 'adyen_gateways'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await adyen_gateways.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '.adyen_payments', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'adyen_payments')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '._disable', async () => {
	  ...
	  const id = TestData.id
		...
		await adyen_gateways._disable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '._enable', async () => {
	  ...
	  const id = TestData.id
		...
		await adyen_gateways._enable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
describe('AdyenGateways resource', () => {
  ...
  it(resourceType + '.create', async () => {
    ...
    const createAttributes = {
			name: 'sigma_9',
			merchant_account: 'lambda_28',
			api_key: 'lambda_97',
			live_url_prefix: 'epsilon_20',
			adyen_payments: [ cl.adyen_payments.relationship(TestData.id) ],
		}

    const attributes = { ...createAttributes, reference: TestData.reference }
    const params = { fields: { adyen_gateways: CommonData.paramsFields } }
    ...
    const intId = cl.addRequestInterceptor((config) => {
      ...
      expect(cl[resourceType].isAdyenGateway(config.data.data)).toBeTruthy()
  ...
  it(resourceType + '.type', async () => {
    ...
    expect(cl[resourceType].isAdyenGateway(resource)).toBeTruthy()
  ...
  /* spec.type.stop */
	...
	it(resourceType + '.adyen_payments', async () => {
		...
		const intId = cl.addRequestInterceptor((config) => {
			expect(config.method).toBe('get')
			checkCommon(config, resourceType, id, currentAccessToken, 'adyen_payments')
			checkCommonParams(config, params)
			return interceptRequest()
		})

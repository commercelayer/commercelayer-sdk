...
import { CommerceLayerClient, ManualTaxCalculator, manual_tax_calculators, tax_rules } from '../../lib'
...
describe('ManualTaxCalculators resource', () => {

  const resourceType = 'manual_tax_calculators'
  const resourcePath = 'manual_tax_calculators'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(manual_tax_calculators)
		expect(manual_tax_calculators.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await manual_tax_calculators.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '.tax_rules', async () => {
	
		const id = TestData.id
		const params = { fields: { tax_rules: CommonData.paramsFields } }
	
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'tax_rules')
			checkCommonParams(request, params)
			return interceptRequest()
		})

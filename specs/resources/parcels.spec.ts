...
describe('Parcels resource', () => {
  ...
  it(resourceType + '.create', async () => {
    ...
    cl.addRequestInterceptor((request) => {
      ...
      expect(cl[resourceType].isParcel(data.data)).toBeTruthy()
  ...
  */
  /* spec.parse.stop */
	...
	it(resourceType + '.package', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourceType, id, currentAccessToken, 'package')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	it(resourceType + '.parcel_line_items', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourceType, id, currentAccessToken, 'parcel_line_items')
			checkCommonParams(request, params)
			return interceptRequest()
		})

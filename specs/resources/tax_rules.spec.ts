...
describe('TaxRules resource', () => {

  const resourceType = 'tax_rules'
  ...
  it(resourceType + '.create', async () => {
    ...
    const intId = cl.addRequestInterceptor((config) => {
      ...
      expect(cl[resourceType].isTaxRule(config.data.data)).toBeTruthy()
  ...
  it(resourceType + '.type', async () => {
    ...
    expect(cl[resourceType].isTaxRule(resource)).toBeTruthy()
  ...
  /* spec.type.stop */
	...
	it(resourceType + '.manual_tax_calculator', async () => {
		...
		const intId = cl.addRequestInterceptor((config) => {
			expect(config.method).toBe('get')
			checkCommon(config, resourceType, id, currentAccessToken, 'manual_tax_calculator')
			checkCommonParams(config, params)
			return interceptRequest()
		})

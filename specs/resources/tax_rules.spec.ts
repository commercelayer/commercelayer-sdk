...
import { CommerceLayerClient, TaxRule, tax_rules, manual_tax_calculators } from '../../lib'
  ...
  /* spec.instance start */
	...
	test(resourceType + '.manual_tax_calculator', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'manual_tax_calculator')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '.event_stores', async () => {
		...
		await tax_rules.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

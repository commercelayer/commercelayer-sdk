...
import { CommerceLayerClient, PriceTier, price_tiers } from '../../lib'
...
describe('PriceTiers resource', () => {
  ...
  const resourcePath = 'price_tiers'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(price_tiers)
		expect(price_tiers.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await price_tiers.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

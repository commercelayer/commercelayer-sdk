...
import { CommerceLayerClient, ExternalPayment, external_payments, orders } from '../../lib'
...
describe('ExternalPayments resource', () => {
  ...
  const resourcePath = 'external_payments'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.wallet', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'wallet')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '.event_stores', async () => {
		...
		await external_payments.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

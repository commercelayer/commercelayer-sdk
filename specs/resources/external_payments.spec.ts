...
describe('ExternalPayments resource', () => {

  const resourceType = 'external_payments'
  ...
  it(resourceType + '.create', async () => {
    ...
    const intId = cl.addRequestInterceptor((config) => {
      ...
      expect(cl[resourceType].isExternalPayment(config.data.data)).toBeTruthy()
  ...
  it(resourceType + '.type', async () => {
    ...
    expect(cl[resourceType].isExternalPayment(resource)).toBeTruthy()
  ...
  it(resourceType + '.parse', async () => {
    ...
    const res = cl[resourceType].parse(payload) as ExternalPayment
  ...
  /* spec.parse.stop */
	...
	it(resourceType + '.wallet', async () => {
		...
		const intId = cl.addRequestInterceptor((config) => {
			expect(config.method).toBe('get')
			checkCommon(config, resourceType, id, currentAccessToken, 'wallet')
			checkCommonParams(config, params)
			return interceptRequest()
		})

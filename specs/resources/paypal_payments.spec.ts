...
import { CommerceLayerClient, PaypalPayment, paypal_payments, orders } from '../../lib'
...
describe('PaypalPayments resource', () => {
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(paypal_payments)
		expect(paypal_payments.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await paypal_payments.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

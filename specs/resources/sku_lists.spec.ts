...
import { CommerceLayerClient, SkuList, sku_lists, customers } from '../../lib'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.bundles', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'bundles')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '.event_stores', async () => {
		...
		await sku_lists.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
import { CommerceLayerClient, OrderSubscriptionItem, order_subscription_items, order_subscriptions, skus, bundles, adjustments } from '../../lib'
...
describe('OrderSubscriptionItems resource', () => {
  ...
  const resourcePath = 'order_subscription_items'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(order_subscription_items)
		expect(order_subscription_items.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.source_line_item', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'source_line_item')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '.event_stores', async () => {
		...
		await order_subscription_items.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

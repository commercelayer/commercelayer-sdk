...
import { CommerceLayerClient, Attachment, attachments, geocoders } from '../../lib'
...
describe('Attachments resource', () => {
  ...
  const resourcePath = 'attachments'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(attachments)
		expect(attachments.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await attachments.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

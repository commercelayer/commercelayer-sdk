...
import { CommerceLayerClient, InventoryStockLocation, inventory_stock_locations, stock_locations, inventory_models } from '../../lib'
...
describe('InventoryStockLocations resource', () => {

  const resourceType = 'inventory_stock_locations'
  const resourcePath = 'inventory_stock_locations'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(inventory_stock_locations)
		expect(inventory_stock_locations.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await inventory_stock_locations.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

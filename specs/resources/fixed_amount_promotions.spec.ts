...
import { CommerceLayerClient, FixedAmountPromotion, fixed_amount_promotions, markets, order_amount_promotion_rules, sku_list_promotion_rules, coupon_codes_promotion_rules, custom_promotion_rules, sku_lists, tags } from '../../lib'
...
describe('FixedAmountPromotions resource', () => {
  ...
  const resourcePath = 'fixed_amount_promotions'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(fixed_amount_promotions)
		expect(fixed_amount_promotions.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await fixed_amount_promotions.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

...
describe('KlarnaGateways resource', () => {
  ...
  const resourcePath = 'klarna_gateways'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await klarna_gateways.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '.klarna_payments', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'klarna_payments')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '._disable', async () => {
	  ...
	  const id = TestData.id
		...
		await klarna_gateways._disable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '._enable', async () => {
	  ...
	  const id = TestData.id
		...
		await klarna_gateways._enable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

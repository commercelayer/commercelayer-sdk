...
import { CommerceLayerClient, AxervePayment, axerve_payments, orders } from '../../lib'
...
describe('AxervePayments resource', () => {
  ...
  const resourcePath = 'axerve_payments'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await axerve_payments.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

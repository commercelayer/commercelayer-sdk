...
import { CommerceLayerClient, PromotionRule, promotion_rules } from '../../lib'
...
describe('PromotionRules resource', () => {
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(promotion_rules)
		expect(promotion_rules.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await promotion_rules.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

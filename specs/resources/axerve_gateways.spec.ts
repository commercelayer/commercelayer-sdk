...
describe('AxerveGateways resource', () => {
  ...
  const resourcePath = 'axerve_gateways'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(axerve_gateways)
		expect(axerve_gateways.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await axerve_gateways.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '.axerve_payments', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'axerve_payments')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '._disable', async () => {
	  ...
	  const id = TestData.id
		...
		await axerve_gateways._disable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '._enable', async () => {
	  ...
	  const id = TestData.id
		...
		await axerve_gateways._enable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

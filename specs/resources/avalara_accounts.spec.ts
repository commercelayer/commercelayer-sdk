...
import { CommerceLayerClient, AvalaraAccount, avalara_accounts, tax_categories } from '../../lib'
...
describe('AvalaraAccounts resource', () => {
  ...
  const resourcePath = 'avalara_accounts'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await avalara_accounts.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

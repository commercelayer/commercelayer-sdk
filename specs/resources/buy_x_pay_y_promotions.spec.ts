...
import { CommerceLayerClient, BuyXPayYPromotion, buy_x_pay_y_promotions, markets, order_amount_promotion_rules, sku_list_promotion_rules, coupon_codes_promotion_rules, custom_promotion_rules, sku_lists, tags } from '../../lib'
...
describe('BuyXPayYPromotions resource', () => {

  const resourceType = 'buy_x_pay_y_promotions'
  const resourcePath = 'buy_x_pay_y_promotions'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await buy_x_pay_y_promotions.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

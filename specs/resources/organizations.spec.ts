...
import { CommerceLayerClient, Organization, organization } from '../../lib'
...
describe('Organizations resource', () => {
  ...
  const resourcePath = 'organization'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(organization)
		expect(organization.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await organization.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

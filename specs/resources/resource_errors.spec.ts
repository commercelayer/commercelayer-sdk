...
import { CommerceLayerClient, ResourceError, resource_errors } from '../../src'
...
describe('ResourceErrors resource', () => {

  const resourceType = 'resource_errors'
  const resourcePath = 'resource_errors'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(resource_errors)
		expect(resource_errors.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await resource_errors.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

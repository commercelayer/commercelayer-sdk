...
import { CommerceLayerClient, CarrierAccount, carrier_accounts, markets } from '../../lib'
...
describe('CarrierAccounts resource', () => {
  ...
  const resourcePath = 'carrier_accounts'
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(carrier_accounts)
		expect(carrier_accounts.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await carrier_accounts.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

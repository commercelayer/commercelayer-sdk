...
describe('PaypalGateways resource', () => {

  const resourceType = 'paypal_gateways'
  const resourcePath = 'paypal_gateways'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await paypal_gateways.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '.paypal_payments', async () => {
		...
		const intId = cl.addRequestInterceptor((request) => {
			expect(request.options.method).toBe('GET')
			checkCommon(request, resourcePath, id, currentAccessToken, 'paypal_payments')
			checkCommonParams(request, params)
			return interceptRequest()
		})
	...
	test(resourceType + '._disable', async () => {
	  ...
	  const id = TestData.id
		...
		await paypal_gateways._disable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))
	...
	test(resourceType + '._enable', async () => {
	  ...
	  const id = TestData.id
		...
		await paypal_gateways._enable(id, {}, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

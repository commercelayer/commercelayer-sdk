...
import { CommerceLayerClient, ReturnLineItem, return_line_items, returns, line_items } from '../../lib'
...
describe('ReturnLineItems resource', () => {
  ...
  */
  ...
  /* spec.instance start */
    ...
    expect(return_line_items)
		expect(return_line_items.type()).toBe(resourceType)
	})
	/* spec.instance stop */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await return_line_items.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

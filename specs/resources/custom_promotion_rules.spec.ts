...
import { CommerceLayerClient, CustomPromotionRule, custom_promotion_rules, percentage_discount_promotions } from '../../lib'
...
describe('CustomPromotionRules resource', () => {

  const resourceType = 'custom_promotion_rules'
  const resourcePath = 'custom_promotion_rules'
  ...
  */
  ...
  /* spec.instance start */
	...
	test(resourceType + '.event_stores', async () => {
		...
		await custom_promotion_rules.event_stores(id, params, CommonData.options)
			.catch(handleError)
			.finally(() => cl.removeInterceptor('request'))

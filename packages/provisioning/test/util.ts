...
export const initConfig = async (env?: string): Promise<CommerceLayerInitConfig> => {
  ...
  if (process.env.CL_PROVISIONING_SDK_ACCESS_TOKEN) {
    accessToken = process.env.CL_PROVISIONING_SDK_ACCESS_TOKEN
    console.log('ENV access token:')
    console.log(accessToken)
  }
  // let organization = process.env.CL_PROVISIONING_SDK_ORGANIZATION

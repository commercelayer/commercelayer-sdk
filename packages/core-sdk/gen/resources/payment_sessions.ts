...
export type PaymentSessionSort = Pick<
  PaymentSession,
  'id' | 'status' | 'amount_cents' | 'currency_code' | 'gift_card_code' | 'expires_at' | 'balance_exceeded_at'
> &
  ResourceSort
// export type PaymentSessionFilter = Pick<PaymentSession, 'id' | 'status' | 'amount_cents' | 'currency_code' | 'gift_card_code' | 'token' | 'vaulting' | 'expires_at' | 'balance_exceeded_at'> & ResourceFilter
...
interface PaymentSession extends Resource {
  ...
  /**
   * The payment session status. One of 'unpaid' (default), 'authorized', 'voided', 'invalidated', 'paid', 'partially_paid', 'refunded', or 'partially_refunded'.
   * @example ```"unpaid"```
   */
  status:
    | 'unpaid'
  ...
  /**
   * The payment session amount cents still to be captured.
   * @example ```1000```
   */
  capture_balance_cents?: number | null
  /**
   * The payment session capture balance, float.
   * @example ```10```
   */
  capture_balance_float?: number | null
  /**
   * The payment session capture balance, formatted.
   * @example ```"€10,00"```
   */
  formatted_capture_balance?: string | null
  /**
   * The payment session amount cents still to be refunded.
   * @example ```1000```
   */
  refund_balance_cents?: number | null
  /**
   * The payment session refund balance, float.
   * @example ```10```
   */
  refund_balance_float?: number | null
  /**
   * The payment session refund balance, formatted.
   * @example ```"€10,00"```
   */
  ...
  token?: string | null
  /**
   * Options to use for the payment session, the passed values will be used to communicate with the payment gateway. Cannot be passed by sales channels.
   * @example ```{"statement_descriptor":"MY SHOP"}```
   */
  options?: Record<string, any> | null
  /**
   * Indicates that the payment instrument used for this session must be stored for future payments. Supported by Adyen, Checkout.com, PayPal, Stripe and external gateways with a token URL.
   * @example ```true```
   */
  vaulting?: boolean | null
  /**
  ...
  /**
   * Time at which this payment session will expire. Set automatically from the payment setting's own expiration window when not provided, and left blank only by the settings that have no gateway behind them.
   * @example ```"2018-01-01T12:00:00.000Z"```
   */
  expires_at?: string | null
  /**
   * Time at which this payment session's authorization was detected to have exceeded the order's remaining balance, if it ever did.
   * @example ```"2018-01-01T12:00:00.000Z"```
   */
  balance_exceeded_at?: string | null

...
export type OrderSubscriptionItemSort = Pick<OrderSubscriptionItem, 'id' | 'quantity' | 'unit_amount_cents'> &
  ResourceSort
// export type OrderSubscriptionItemFilter = Pick<OrderSubscriptionItem, 'id' | 'quantity' | 'unit_amount_cents'> & ResourceFilter

/**
 * The Order subscription item object is returned as part of the response body of each successful list, retrieve, create, update or delete API call to the /api/order_subscription_items endpoint.
 *
 * @link https://docs.commercelayer.io/core-api-reference/order_subscription_items/object
 */
...
class OrderSubscriptionItems extends ApiResource<OrderSubscriptionItem> {
  ...
  async create(
    resource: OrderSubscriptionItemCreate,
    params?: QueryParamsRetrieve<OrderSubscriptionItem>,
    options?: ResourcesConfig,
  ): Promise<OrderSubscriptionItem> {
    return this.resources.create<OrderSubscriptionItemCreate, OrderSubscriptionItem>(
      { ...resource, type: OrderSubscriptionItems.TYPE },
      params,
      options,
    )
  }
  ...
  async update(
    resource: OrderSubscriptionItemUpdate,
    params?: QueryParamsRetrieve<OrderSubscriptionItem>,
    options?: ResourcesConfig,
  ): Promise<OrderSubscriptionItem> {
    return this.resources.update<OrderSubscriptionItemUpdate, OrderSubscriptionItem>(
      { ...resource, type: OrderSubscriptionItems.TYPE },
      params,
      options,
    )
  ...
  async order_subscription(
    orderSubscriptionItemId: string | OrderSubscriptionItem,
    params?: QueryParamsRetrieve<OrderSubscription>,
    options?: ResourcesConfig,
  ): Promise<OrderSubscription> {
    const _orderSubscriptionItemId =
      (orderSubscriptionItemId as OrderSubscriptionItem).id || (orderSubscriptionItemId as string)
    return this.resources.fetch<OrderSubscription>(
      { type: 'order_subscriptions' },
      `order_subscription_items/${_orderSubscriptionItemId}/order_subscription`,
      params,
      options,
    ) as unknown as OrderSubscription
  }

  async source_line_item(
    orderSubscriptionItemId: string | OrderSubscriptionItem,
    ...
    return this.resources.fetch<LineItem>(
      { type: 'line_items' },
      `order_subscription_items/${_orderSubscriptionItemId}/source_line_item`,
      params,
      options,
  ...
  async event_stores(
    ...
    return this.resources.fetch<EventStore>(
      { type: 'event_stores' },
      `order_subscription_items/${_orderSubscriptionItemId}/event_stores`,
      params,
      options,

...
import { inbound_messages } from '../api'
import type { InboundMessage } from '../model'
...
describe('InboundMessages resource', () => {
  const resourceType = 'inbound_messages'
  const resourcePath = 'inbound_messages'
  ...
  test(resourceType + '.retrieve', async () => {
    ...
    await inbound_messages
      .retrieve(id, params, CommonData.options)
      .then((res: InboundMessage) => expect(res).not.toBeNull())
      .catch(handleError)
  ...
  test(resourceType + '.type', async () => {
    const resource = { id: TestData.id, type: 'inbound_emails' }
    expect(inbound_messages.isInboundMessage(resource)).toBeTruthy()

    const type = inbound_messages.type()
  ...
  test(resourceType + '.relationship', async () => {
    const relId = inbound_messages.relationship(TestData.id)
    expect(isDeepStrictEqual(relId, { id: TestData.id, type: resourceType }))

    const relResId = inbound_messages.relationship({ id: TestData.id, type: resourceType })
  ...
  test(resourceType + '.parse', async () => {
    ...
    const res = inbound_messages.parse(payload) as InboundMessage
  ...
  */
  ...
  test(resourceType + '.instance', async () => {
    expect(inbound_messages)
    expect(inbound_messages.type()).toBe(resourceType)
  })
  /* spec.instance stop */

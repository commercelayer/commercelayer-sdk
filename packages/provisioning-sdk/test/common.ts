...
import type { RequestObj } from '@runtime/interceptor'
...
import {
  ...
  type CommerceLayerProvisioningBaseClient,
...
const initClient = async (config: CommerceLayerConfig): Promise<CommerceLayerProvisioningBaseClient> => {
  ...
  try {
    vi.setConfig({ testTimeout: config.timeout || GLOBAL_TIMEOUT })
  } catch (_err: any) {}
...
const getClient = (config?: CommerceLayerConfig): Promise<CommerceLayerProvisioningBaseClient> => {
...
const checkCommonData = (data: any, type: string, attributes: any, id?: string) => {
  ...
  Object.entries(data.data.relationships).forEach(([k, v]) => {
    relationships[k] = (v as any).data
  })
  const received = {
...
const checkCommonParamsList = (request: RequestObj, params: QueryParamsList<Resource>) => {
  ...
  if (params.sort) {
    ...
    if (Array.isArray(params.sort)) value = params.sort.join(',')
      ...
      value = Object.entries(params.sort)
        .map(([k, v]) => `${v === 'desc' ? '-' : ''}${k}`)
        .join(',')

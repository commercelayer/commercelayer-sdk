...
class CommerceLayerProvisioningBaseClient {
  readonly apiSchemaVersion = API_SCHEMA_VERSION
  readonly apiSupportedVersions = API_SUPPORTED_VERSIONS

  protected static cl: CommerceLayerProvisioningBaseClient

  static get(config?: CommerceLayerInitConfig): CommerceLayerProvisioningBaseClient {
    if (config) return (CommerceLayerProvisioningBaseClient.cl = new CommerceLayerProvisioningBaseClient(config))
    else if (!CommerceLayerProvisioningBaseClient.cl)
      throw new Error('CommerceLayerProvisioning client not initialized')
    return CommerceLayerProvisioningBaseClient.cl
  }

  // The ResourceAdapter created for this specific client instance. Every
  // `CommerceLayerProvisioning(config)` call builds a fresh adapter (its own ApiClient
  ...
  config(config: CommerceLayerConfig): this {
    ...
    // CommerceLayerProvisioning config
...
const CommerceLayerProvisioning = (config: CommerceLayerInitConfig): CommerceLayerProvisioningBaseClient => {
  return CommerceLayerProvisioningBaseClient.get(config)
}

export default CommerceLayerProvisioning
export type { CommerceLayerConfig, CommerceLayerInitConfig }
export { CommerceLayerProvisioning, CommerceLayerProvisioningBaseClient }

...
class Prices extends ApiResource<Price> {
  ...
  async price_tiers(
    priceId: string | Price,
    params?: QueryParamsList<PriceTier>,
    options?: ResourcesConfig,
  ): Promise<ListResponse<PriceTier>> {
    const _priceId = (priceId as Price).id || (priceId as string)
    return this.resources.fetch<PriceTier>(
      { type: 'price_tiers' },
      `prices/${_priceId}/price_tiers`,
      params,
      options,
    ) as unknown as ListResponse<PriceTier>
  }

  async price_volume_tiers(
    priceId: string | Price,
    params?: QueryParamsList<PriceVolumeTier>,
    options?: ResourcesConfig,
  ): Promise<ListResponse<PriceVolumeTier>> {
    const _priceId = (priceId as Price).id || (priceId as string)
    return this.resources.fetch<PriceVolumeTier>(
      { type: 'price_volume_tiers' },
      `prices/${_priceId}/price_volume_tiers`,
      params,
      options,
    ) as unknown as ListResponse<PriceVolumeTier>
  }

  async price_frequency_tiers(
    priceId: string | Price,
    params?: QueryParamsList<PriceFrequencyTier>,
    options?: ResourcesConfig,
  ): Promise<ListResponse<PriceFrequencyTier>> {
    const _priceId = (priceId as Price).id || (priceId as string)
    return this.resources.fetch<PriceFrequencyTier>(
      { type: 'price_frequency_tiers' },
      `prices/${_priceId}/price_frequency_tiers`,
      params,
      options,
    ) as unknown as ListResponse<PriceFrequencyTier>
  }
  ...
  async jwt_stock_locations(
    ...
    return this.resources.fetch<StockLocation>(
      ...
      `prices/${_priceId}/jwt_stock_locations`,
      params,
      options,

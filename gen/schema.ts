...
const parseSchema = (path: string, _opts?: { apiHost?: string; apiVersion?: string }): ApiSchema => {
  ...
  Object.keys(apiSchema.paths).forEach((p) => {
    ...
    Object.keys(apiSchema.components).forEach((c) => {
      ...
      else if (Inflector.snakeCase(c.replace(specialComponentMatcher, '')) === singRes)
        resources[p].components[c] = apiSchema.components[c]
    })
    // Sort components
...
const operationName = (op: string, id?: string, relationship?: string): string => {
  switch (op) {
    case 'get':
      return id ? relationship || 'retrieve' : 'list'
    case 'patch':
      return 'update'
    case 'delete':
      return 'delete'
  ...
  for (const p of Object.entries(schemaPaths)) {
    ...
    Object.entries(pValue as object).forEach((o) => {
      ...
      if (oValue.responses) {
        ...
        else if (oValue.responses['201']?.content) op.responseType = referenceContent(oValue.responses['201'].content)
      }
  ...
  for (const c of Object.entries(schemaComponents)) {
    ...
    Object.entries(cmpAttributes.properties as object).forEach((a) => {
      ...
      attributes[aKey] = {
        ...
        required:
          requiredAttributes.includes(aKey) || (fetchable && !aValue.nullable && !cKey.match(/(Create|Update)$/)),
        fetchable,
        enum: aValue.enum,
